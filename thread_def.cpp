...
void *say_hello(void *args) {
    int *tid_str = static_cast<int *>(args);
    string hello_str = "hello, world! ";
    hello_str += (std::to_string(*tid_str));

    hello_str += std::to_string(getpid());
    hello_str += "\n";
    cout << hello_str;
    return 0;
}
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        int x = i;
        int ret = pthread_create(&tids[i], NULL, say_hello, &x);
        if (ret != 0) {
